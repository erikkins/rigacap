-- SqlProcedure: [dbo].[watchSell20Up]
...
if @channel is null
	...
	begin
			...
			if @lastPrice >= @buyprice *1.2
				begin
					--sell off all!
					select @audit = @ticker + ' Price exceeded 120%, selling all'
					exec saveAudit @now,  @audit, @stockID

					exec addSell @buyID, @watchID, @lastDataDate, 100, 'Price exceeded 120% of buy, selling all'
				end
