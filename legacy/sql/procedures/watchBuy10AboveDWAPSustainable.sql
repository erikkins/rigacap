-- SqlProcedure: [dbo].[watchBuy10AboveDWAPSustainable]

declare @watchID int

select @watchID = watchID 
from Watch 
where ProcName = 'watchBuy10AboveDWAPSustainable'
...
fetch next from si2 into @sid, @atwhen
	while @@fetch_status=0
		...
		if exists (select * from stockdata where stockID=@sid and atwhen > @atwhen and price >= @xprice * 1.1)
			begin																																								--vol was 1500000
				select top 1 @buyprice = price, @buyDate=atwhen, @buyVol = volume from stockdata where stockID=@sid and atwhen > @atwhen and price >=@xprice * 1.1 and volume > 1000000 order by atwhen asc
				...
				select @DWAPAtBuy = dbo.fnDWAP(@sid, @BuyDate)				
				select @DWAPDWAP = dbo.fnDWAP(@sid, @atwhen)
				select @goodCurve=1
				
				--if the 50DMA @ DWAP Cross is above the line, but crosses under, then don't buy
				--if @50DMAPX > @DWAPDWAP and @50DMAP < @DWAPAtBuy
				--	begin
				--		select @goodCurve=0
				--	end						

	
				--before there was also a @buyprice > 25 requirement      --was @dwapatbuy * 1.02
				if @buyprice > @200DMAP and @avg > 1000000 and @buyprice > @DWAPAtBuy and @goodcurve=1-- and @buyVol > @xvol and @xprice > @50DMAPX														
						BEGIN				
