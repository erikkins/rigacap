-- SqlProcedure: [dbo].[watchSell8Down]
		...
		if @multLow is null
			begin
				...
				if @lastprice <= @buyprice * .92
					begin
						exec saveAudit @selldate, 'Stock dropped 8% below buy price', @stockID
						exec addSell @buyID, @watchID, @selldate, @shares, 'Stock dropped 8% below buy price'
					end
			end
		else
			begin
