-- SqlProcedure: [dbo].[getDWAPriceIO]
...
declare @finaldwap money
select @finaldwap = convert(money, sum(dwa)) from #td
if not exists (select * from DataCache where stockID=@StockID and AtWhen=@dateofinterest)
	begin
		if @price > 0
			begin
				if @finaldwap is not null
					begin
						--Print 'inserting ' + convert(varchar,@StockID) + ' for ' + Convert(varchar,@dateofinterest) + ' at ' + convert(varchar,@Price)
						insert into DataCache(StockID, AtWhen, Price, DWAP, RDWAP, CacheDate)							
							select @StockID, @dateofinterest, @Price, @finaldwap ,null, @today
	...
	begin
		if @price > 0
			begin				
				--Print 'updating ' + convert(varchar,@StockID) + ' for ' + Convert(varchar,@dateofinterest) + ' at ' + convert(varchar,@Price)
				if @finaldwap is not null
					begin
					update DataCache
					set price = @price,
					...
					cachedate = @today 
					where StockID = @StockID
					and DataCache.AtWhen = @dateofinterest
					end
			end
	end
