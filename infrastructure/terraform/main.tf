...
resource "aws_lambda_function" "worker" {
  ...
  environment {
    variables = merge(local.lambda_env_vars, {
      LAMBDA_ROLE          = "worker"
      # SIGNAL_UNIVERSE_SIZE = 200 matches the canonical marketing baseline
      # (8-date 5y avg, +160.22% / 21.5% ann / 0.92 Sharpe / 20.4% MaxDD,
      # generated by scripts/launch-5y-8dates.sh on 2026-04-28). That script
      # invokes scripts/local_wf_runner.py with --max-symbols 200.
      #
      # PRIOR HISTORY:
      # - was 100 before May 17 2026 (too restrictive vs WF; ~70% of WF trades
      #   came from rank 101-500 in the wrong-baseline audit)
      # - bumped to 500 on May 17 based on liquidity-rank audit of job 1248,
      #   which turned out NOT to be the marketing-canonical run
      # - corrected to 200 on May 18 to match the actual marketing baseline
      SIGNAL_UNIVERSE_SIZE = "200"
...
# ============================================================================
# Monthly universe audit — full Alpaca reconciliation belt-and-suspenders
# beyond the nightly delta. 1st of every month at 3 AM EDT (07:00 UTC).
# ============================================================================

resource "aws_cloudwatch_event_rule" "monthly_universe_audit" {
  name                = "${local.prefix}-monthly-universe-audit"
  description         = "Monthly full-universe reconciliation against Alpaca"
  schedule_expression = "cron(0 7 1 * ? *)"
}

resource "aws_cloudwatch_event_target" "monthly_universe_audit" {
  rule      = aws_cloudwatch_event_rule.monthly_universe_audit.name
  target_id = "lambda-monthly-universe-audit"
  arn       = aws_lambda_function.worker.arn
  input     = jsonencode({ monthly_universe_audit = true })
}

resource "aws_lambda_permission" "monthly_universe_audit" {
  statement_id  = "AllowMonthlyUniverseAuditEventBridge"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.worker.function_name
  principal     = "events.amazonaws.com"
...
resource "aws_cloudwatch_event_rule" "quarterly_deep_audit" {
  name                = "${local.prefix}-quarterly-deep-audit"
  description         = "Quarterly deep audit: monthly + 90d corp-actions replay"
  schedule_expression = "cron(0 8 1 1,4,7,10 ? *)"
}

resource "aws_cloudwatch_event_target" "quarterly_deep_audit" {
  rule      = aws_cloudwatch_event_rule.quarterly_deep_audit.name
  target_id = "lambda-quarterly-deep-audit"
  arn       = aws_lambda_function.worker.arn
  input     = jsonencode({ quarterly_deep_audit = true })
}

resource "aws_lambda_permission" "quarterly_deep_audit" {
  statement_id  = "AllowQuarterlyDeepAuditEventBridge"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.worker.function_name
  principal     = "events.amazonaws.com"
