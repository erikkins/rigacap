...
**Fourth gotcha — headless Chrome viewport clips ~88px from `--window-size`:** at `--window-size=1080,1350`, Chrome's actual rendered viewport tops out near y=1262, silently clipping the bottom ~88px of the page. The pullquote (which sits at y=1189-1290 by flex layout) ends up half-cut even though the layout is correct. Layout JS measurement returns `pq.bottom = 1290` while the PNG only contains paper past y=1262 — proof the screenshot, not the layout, is the cause. **Fix: render at `--window-size=1080,1500` and crop the PNG to (0,0,1080,1350) with PIL.** Verifying: after fix, content should reach y≈1289 on cards with bottom-anchored elements.
