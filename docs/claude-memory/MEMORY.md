...
- **[Parquet divergence blocking Stage 3b cutover (May 8)](project_parquet_divergence_blocker.md)** — Stage 3a diff harness shows 4632 structural divergences (~every symbol). Cannot cut to parquet primary until diff is zero. Triage path + most-likely root causes (partition predicate mismatch, indicator-column drift, end-of-history boundary) in linked file.
- **[Dashboard Portfolio Banner — user-personal, not model-portfolio (Tue May 5)](project_dashboard_personal_portfolio.md)** — current banner shows MODEL portfolio's $42K, confusing subscribers into thinking it's theirs. Fix: same visual treatment, per-user simulation cached nightly. Schema migration first, then code. ~half day.
