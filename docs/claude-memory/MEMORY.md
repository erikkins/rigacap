...
- **[Close direct execute-api WAF bypass](project_execute_api_waf_bypass.md)** — ~30 min hardening pass: CloudFront injects `X-Origin-Verify` header; FastAPI middleware rejects requests without it. After May 3 cutover, api.rigacap.com is WAF-protected, but raw execute-api URL still publicly reachable. Low-medium priority.
