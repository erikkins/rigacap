...
- **[A1 failed → signal quality is the bottleneck (Jun 1 2026)](project_a1_failed_signal_quality_bottleneck.md)** — First v2-compliant variant (4×22% + T3) FAILED dramatically (5.96% ann, 29.7% MDD). Concentration destroys the strategy because rank-4 signal ≈ rank-6 signal in expected return. "Fewer better" needs "better" FIRST. Next hypotheses: Categories D (new signal features) + E (regime sub-strategies) + universe-filter quality. Don't retest concentration until signal differentiation improves.
