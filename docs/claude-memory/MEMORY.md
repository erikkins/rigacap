...
## ▶▶ RESUME HERE (Jun 9 2026) — LAUNCHED, but t30v is DEFENSE not offense (read first)
- **[Inversion campaign — naive CAN'T be defended cheaply (Jun 9 night)](project_inversion_campaign_jun9.md)** — 17 variants (naive 28.8% base + trail/200MA/DD-tighten/gates/factor-trend): NONE got daily MDD <40%. Naive's TRUE daily MDD = −52% (race JSON's −30% = monthly-sampling artifact — rebuild before animation). 2021-23 factor unwind is index-invisible. t30v's 17.9% through same unwind = gate conjunction is the moat. Behavioral: t30v beats panic-SPY ALWAYS, never beats panic-naive. Erik: keep 14% copy live meanwhile. t30v ablation (which gate = the DD tech) running → `scripts/ablation_results.json`.
