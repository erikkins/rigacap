...
## 🧹 TOMORROW'S LIST (Jun 23 EOD) — 3 deliberate sweeps
1. **CLOSE THE LOOP (parquet base freshness)** — port PITFWU read+append from scripts/ into backend/app (NOT in Lambda image currently), wire daily append (additive/safe), flag-gate PITFWU scoped read + shadow-diff (PITFWU==pickle to the penny historically), cut over, retire all_data.parquet for daily. Also fixes the daily-WF stale-data (reads frozen Jun-15 parquet). Staged like the original flip.
