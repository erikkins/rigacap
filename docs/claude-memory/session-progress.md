...
## 🚀 IN FLIGHT — SCHEDULE LAUNCH CARDS + PUSH #1: launch cards live at https://rigacap.com/launch-cards/launch-1..5.png (HTTP 200). Mechanism: POST /api/admin/social/posts/compose (platform, text_content, image_s3_key=full CloudFront URL ok, status=approved) → POST /api/admin/social/posts/{id}/publish. Toggle: X+IG+TikTok ON, Threads OFF. AWAITING Erik "send it" to fire card #1 to X (drafted caption: "After fifteen years building it quietly, RigaCap is live... Preserver to protect, Maximizer to push... rigacap.com/x/track-record"). Then schedule cards 2-5 spread over days across X+IG. WIDGET (tier-aware /app) = PARKED by Erik.
