...
2. **Route admin alerts to push** — backend `push_notification_service.send_to_user(...)` exists; wire new-account/0-signal-scan/hygiene email alerts to also push to Erik's user_id with `data.screen` (users/portfolio) for deep-link.
