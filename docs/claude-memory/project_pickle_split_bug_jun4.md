...
**PROOF of fix:** split-adjusting the pickle (back-divide pre-split px by N) removed the phantom basket trades. BUT — the heuristic ratio-detector adjuster was itself WRONG (see correction below).

**CORRECTION (Jun 5, via verified PITFWU):** the heuristic "split-adjusted → 26.2% ann / 20.6% MDD" number was ITSELF an ARTIFACT — inflated. The ratio-near-round-factor detector false-positived real crashes as splits (a −50% day == a 2:1 split ratio), divided away the loss, pumped returns. The verified PITFWU corp-actions adjuster (yfinance corr 1.0) only touches ACTUAL splits. On clean, corp-actions-correct, survivorship-free PITFWU data: **M3 bear-inclusive ≈ 13.8% ann / 0.74 / 27% MDD** (v1, fixed-start universe). Survivor-only pool = 15.8% (survivorship ≈ +2pp for top-100; it flatters Sharpe 0.74→0.89 + MDD 27→21 more). So ALL THREE prior numbers were wrong: buggy-raw 18.6%/38% (phantom losses down), heuristic 26%/20.6% (false-split-erased crashes up), cited-memory 21.5% MDD. The REAL clean number (~14% ann / 27% MDD) is more modest than every estimate. Authoritative bench (per-period + VB-carry) still pending but won't move this much.
